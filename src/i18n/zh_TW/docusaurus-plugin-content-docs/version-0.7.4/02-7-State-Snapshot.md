...
一個處於啟動狀態的 Claptrap ，它的 State 即是當前的最新數據狀態。这是通过事件溯源的方式来从持久化层还原的。有時，事件的數量可能非常龐大。那么通过事件来还原 State 将会花费更多的时间。因此，在 Claptrap 框架中提供了狀態快照來持久化特定 Claptrap 在一定條件之後的狀態。這個條件通常來說是以下幾種：
...
不過，快照的存在也会带来一些特殊性。結合上面的工作步驟，我們很容易就發現，一旦形成了快照：
