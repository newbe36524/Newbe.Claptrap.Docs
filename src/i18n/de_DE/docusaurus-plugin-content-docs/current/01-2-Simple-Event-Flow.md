...
Erstellen Sie`<code>RemoveAllItems FromCartEventHandler`-Klasse unter dem Ordner`Cart/Events`Ordner des `HelloClaptrap.Actor`-Projekts.
...
Öffnen Sie den `CartController` unter dem `OrdnerController` für `helloClaptrap.web`-Projekt.
