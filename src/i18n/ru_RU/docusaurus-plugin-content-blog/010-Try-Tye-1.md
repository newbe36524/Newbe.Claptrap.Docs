...
Например, приложение net5 TFM, используемое автором в этом использовании, поэтому необходимо сначала потянуть`mcr.microsoft.com/dotnet/aspnet:5.0`в качестве базового зеркала.

Поскольку источник базового зеркала netcore теперь перенесен с docker hub на mcr.microsoft.com.Поэтому рекомендуется использовать`Newbe.McrMirror`для ускоренной загрузки.
