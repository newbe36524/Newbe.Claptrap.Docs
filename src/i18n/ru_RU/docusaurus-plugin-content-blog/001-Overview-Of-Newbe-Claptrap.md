...
**имеет собственное состояние**.Объекты могут иметь свои собственные свойства, которые являются основными функциями объектно-ориентированного языка.在 Actor 模式中，这些属性都被统称为 Actor的状态（State）。Состояние Actor поддерживается самим Actor.
